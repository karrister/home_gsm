...
void logger(char *stringbuf)
{
#if 0
	lcd.print(stringbuf);
#endif
	;//whatever to do for this message
}
...
wait_for_ok:
	...
	do
	{
		...
		{
			 //Flash screen to let user know we are not stuck
...
void do_action(int opcode)
{
	...
	switch(opcode)
	{
		 ...
		 case CMGR:
			 gsm_print("",0);
			 lcd.setCursor(0, 0);
			 logger(t_smsheaders.number); 
			 delay(2000);
			 gsm_print("",1);
			 lcd.setCursor(0, 1);
			 logger(t_smsheaders.date); 
			 lcd.setCursor(9, 1);
			 logger(t_smsheaders.time); 
			 delay(3500);
